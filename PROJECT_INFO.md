...
| シークレットジェスチャー | 🃏長押し3秒→3回タップで/mへ遷移（隠し機能） |
...
│   └── useSecretGesture.ts # シークレットジェスチャーフック
├── types/
│   └── index.ts          # 型定義
└── index.ts              # 公開API
```
...
| 2025-02-06-001 | シークレットジェスチャー・状態管理 |
...
1. **Git Tag v1.0.0** - `git checkout v1.0.0` で復元可能
2. **ファイル保護** - パス保護（check-protected-features.js）+ SHA256チェックサム（protect-game-checksums.js）
3. **ロックダウンテスト** - 定数値・API表面・ゲームロジックの自動検証

**バイパス方法:**

| 方法                 | コマンド                                    |
| -------------------- | ------------------------------------------- |
| チェックサムスキップ | `ALLOW_GAME_CHANGES=1 git commit`           |
| パス保護スキップ     | コミットメッセージに `--allow-game-changes` |
| 緊急バイパス         | コミットメッセージに `--emergency-override` |
| チェックサム更新     | `pnpm protect:game:update`                  |

**保護コマンド:**

| コマンド                   | 説明               |
| -------------------------- | ------------------ |
| `pnpm protect:game`        | チェックサム検証   |
| `pnpm protect:game:init`   | チェックサム初期化 |
| `pnpm protect:game:update` | チェックサム再計算 |
| `pnpm check:protected`     | パス保護チェック   |
...
完全な技術仕様は [SPECIFICATION.md](./SPECIFICATION.md) のセクション16-19を参照
