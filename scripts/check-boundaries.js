...
// 許可された例外パターン（認証インフラ等、App Layerに公開が必要なもの）
const allowedExports = {
  フック公開: [
    // AuthProviderのuseAuthContextはApp Layer認証インフラとして許可
    { feature: 'messenger', pattern: /useAuthContext/ },
  ],
  UIコンポーネント公開: [
    // GameBoardはゲームフィーチャーの公開API
    { feature: 'game', pattern: /GameBoard/ },
    // AuthProviderはApp Layer認証インフラとして許可
    { feature: 'messenger', pattern: /AuthProvider/ },
  ],
}
...
function checkFile(filePath, content, featureName) {
  ...
  for (const check of checkPatterns) {
    ...
    if (matches) {
      // Check if this violation is in the allowed exceptions list
      const allowed = allowedExports[check.name]
      if (allowed) {
        const isAllowed = allowed.some(
          (rule) => rule.feature === featureName && rule.pattern.test(content)
        )
